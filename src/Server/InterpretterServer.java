...
public class InterpretterServer {
    ...
    public Message Do(Message message) throws SQLException {
        ...
        switch (message.getCommand()) {
            case "Mail":
                ...
                return new Message("Mail", message.getAccount(), status);

            case "LogIn":
                status = logIn(message);
                return new Message("LogIn", message.getAccount(), status);

            case "Register":
                status = register(message);
                return new Message("Register", message.getAccount(), status);
            ...
            case "DeleteAccount":
                ...
                return new Message("DeleteAccount", message.getAccount(), status);

            case "DeleteMail":
                status = deleteMail(message);
                return new Message("DeleteMail", message.getAccount(), status);

            case "ChangePassword":
                status = changePassword(message);
                return new Message("ChangePassword", message.getAccount(), status);

            default:
                return new Message("Fail", message.getAccount(), false);
        }
    }
    ...
    public boolean deleteMail(Message message) throws SQLException {
        ...
        return connEmails.deleteMess(account, id);
    }

    public Message sendMails(Message message) throws SQLException {
        String account = message.getAccount();
        return connEmails.getListMess(account);
    }
